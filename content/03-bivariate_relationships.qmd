...
As usual, we start with an interesting question. You have some outcome of interest and you think that there is an important determinant of that outcome that no has yet identified. Or perhaps that the relationship between some heavily chewed over determinant and the outcome of interest is misunderstood. We are steadily building up your ability to determine empirically the relationship between that determinant and the outcome of interest.

Last week, we discussed various tools that you can use to explore your data. You can develop a very good understanding of each of the individual variables, both dependent and independent. This exploration is very important for building your intuition and, by extension, your expertise in the question at hand. These tools also allow you to identify and deal with unusual data points (or outliers).

This week, we will start to get to the heart of empirical analysis: understanding the relationships between variables. We have an outcome of interest (the dependent variable) and we have a theory about how a factor or set of factors (independent variables) related to that outcome. Simply put (and there really is no need to over-complicate this), we want to understand 1) how these variables change in relation to one another and 2) whether this relationship is strong enough that we should declare (though 8,000 word journal articles or by shouting from rooftops) that whenever we want to change or understand that outcome, we *must* consider these important independent variables.

We will start small, looking at how we can measure the relationship between two variables. Later, we will explore how we can measure the relationship between many variables. Rarely are the complex outcomes that concern us as political scientists determined by only one factor. In your research you will need to consider the many important determinants of the outcome. However, it is very important to consider the relationship between each of these individual independent variables and your dependent variable. How do they move with each other: when one goes up, does the other go down, up, or not really move? How strong is this association?

This exploration is particularly important for you to do with regard to the independent variable(s) that are the focal point of your theory and, therefore, your contribution to our understanding of the spaghetti bowl of things that determine your outcome of interest. Just as it is important for you to spend some time understanding the shape of your variables (using the tools we discussed last week), you must start to understand the shape of the relationship between your dependent variable and independent variables.

Let's begin!

### Bivariate relationships

How do two variables move with one another? When when goes up, does the other go down, up, or not really move at all? How dramatic is this shift? The type of variables we have determines how we can answer this question. To begin, we will explore the relationship between two continuous variables. Later in the class, we will look at how to explore the relationship between a continuous and categorical variable.
...
> TODO: Include an explanation of the Gapminder research question.
...
> You can transform your data to make it easier to work with. Just remember that you now need to talk in terms of logged GDP per capita instead of GDP per capita. You can, of course, transform you logged variable back to its raw values to ease your interpretation (I cannot, for the life of me, understand logs and I exert a lot of effort towards spending as little time with them as possible).
...
Looking at our data above, it appears that a straight line is the best line to draw (perhaps because that's the type of line I asked you to consider).

> **Overfitting** involves fitting a model (or drawing a line through our data) that misses the forest for the trees. You can draw all kinds of shapes through those data that perhaps result in a smaller distance between itself and each dot. In fact, if you draw a line that connects all of those dots like some kind of spaghetti bowl there will be no difference between your line and the data points. However, this model may be too focused on the data we have at hand. Introduce a new data point, and our model has no idea what to do with it. This is bad! Your aim here is to produce a generalizable model of the relationship between these two variables, not to draw a line that connects this particular constellation of dots.
>
> We will talk more about overfitting later in the semester.
...
So, the constant that best predicts a country's average life expectancy based on its logged GDP per capita is equal to the average life expectancy across our sample (`r mean(gapminder_df$life_exp) |> round(2)` years) minus the average logged GDP per capita (`r mean(gapminder_df$log_gdp_per_cap) |> dollar()`) transformed by $\beta_1$.
...
This line looks good! Of course, not all the dots line up. That's okay. We want something generalizable. A bit of noise (error) is expected. How much error is okay? This is a complicated question that has contested answers. Let's start with actually measuring that error. Then we can chat about whether or not it's small enough to allow us to be confident in our model.
...
Start by working out the average life expectancy predicted by our model for the logged GDP per capita of all of our countries. We can then compare this the average life expectancy actually observed in all these countries. We can predict values from a model using `broom::augment()`:
...
This is useful, but influenced by the units by which we measure our variables. What if we want to compare model performance?
