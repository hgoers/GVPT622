...
You will need access to the following R packages to complete this week's activities. If you have not already done so, please install or update them by running the following:
...
You will then need to load them into your current session:
...
You can use the skills we will discuss in this section to help you answer these questions. For example, you can determine whether there are relatively few young voters compared to older voters. Then you can explore why this might be. In turn, your growing expertise in and understanding of the data generation process should inform your exploration of the data. You might note that people have to wait in long queues on a Tuesday to vote. Does this impact the number of workers vs. retirees who vote?
...
Let's use the [American National Election Survey](https://electionstudies.org/) to explore how to produce useful descriptive statistics for categorical variables using R. The ANES polls annually individual Americans about their political beliefs and behavior.
...
Take a look at the many different bits of information collected about each respondent by running `?nes` in your console.
:::
...
This variable takes on six different values, recording the age range into which the respondent to the NES falls:
...
-   We are missing `r nes |> filter(is.na(dem_age6)) |> nrow()` observations (in other words, `r skimr::skim(nes$dem_age6)$n_missing` people did not provide their age when asked);
-   This means that we have information on `r scales::percent(skimr::skim(nes$dem_age6)$complete_rate)` of our observations (from `complete_rate`);
-   The package authors have incorrectly coded this variable as unordered;
-   We have `r skimr::skim(nes$dem_age6)$factor.n_unique` different categories.

::: callout-tip
Remember, there are many different data types that R recognizes. These include characters (`"A"`, `"B"`, `"C"`), integers (`1`, `2`, `3`), and logical values (`TRUE` or `FALSE`). R treats categorical variables as **factors**.

Happily, the authors of the `poliscidata` package coded this variable as a factor. However, they have left it unordered (see the `ordered` variable in the printed out summary). We can easily convert this into an ordered factor:
...
Here, we used the `ordered` argument for the `dplyr::mutate()` function to tell R that this factor is ordinal. We then used the `levels` argument to tell R the order of the categories.
...
There are three broad distributions that you should know: normal, right-skewed, and left-skewed.

#### Normal distribution

A normally distributed variable includes values that fall symmetrically from their center point, which is the peak (or most common value).

Examples of normally distributed data include the height or weight of all individuals in a large population.

::: callout-note
This distribution is sometimes also referred to as a bell-curve.
:::
...
The box in the graph above displays the 25th percentile, the median, and the 75th percentile values. The tails show you all the data up to a range 1.5 times the **interquartile range (IQR)**, or the 75th percentile minus the 25th percentile (or the upper edge of the box minus the lower edge of the box). If the smallest or largest values fall below or above (respectively) 1.5 times the IQR, the tail ends at that value.

### Outliers

Note that some values are displayed as dots. The box plot above is providing you with a bit more information than the five number summary alone. If the data include values fall outside of the IQR, they are displayed as dots. These are (very rule of thumb, take with a grain of salt, please rely on your theory and data generation process instead!) candidates for **outliers**.
...
Imagine we have two datasets, `wide_dist` and `narrow_dist`. Both datasets have the same mean (0) and the same number of observations (1,000,000).
