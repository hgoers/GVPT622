...
```{r}
gdp_75th <- skim(gdp_df) |> 
  filter(skim_variable == "gdp") |> 
  pull(numeric.p75)

gdp_df <- gdp_df |> 
  mutate(income_level = if_else(gdp >= gdp_75th, "High", "Medium or low"),
...
medals_avg <- medals_gdp_df |> 
  ...
  summarise(n_countries = n(), 
            avg_medals = mean(medals_won, na.rm = T)) |> 
  mutate(diff_means = avg_medals - lead(avg_medals))

medals_avg
```

It certainly looks like high-income countries won more medals on average rather than medium- and low-income countries. This difference looks pretty substantial, but we still need to confirm that this difference is not due to random error. 
...
We now have 5,000 simulated differences in the average number of medals won by high-income countries and medium- and low-income countries for our 5,000 samples from our simulated null world. The only thing creating these differences in random noise. 

#### Step 5: Work out whether our sample could plausibly exist in this null world

We now have a good idea of the distribution of medals between high-income countries and medium- and low-income countries in a null world in which there was no difference between these groups. 
...
In our null world, 95% of the differences in means between countries with these two income levels sit between `r round(abs(boostrapped_confint$lower_ci), 1)` more medals won by high-income countries and `r round(boostrapped_confint$upper_ci, 1)` more medals won by medium- and low-income countries, on average.
